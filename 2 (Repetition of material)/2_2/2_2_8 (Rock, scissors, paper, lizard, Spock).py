# TODO Камень, ножницы, бумага, ящерица, Спок 🌶️
#
# Проиграв 10 раз Тимуру, Руслан понял, что так дело дальше не пойдет,
# и решил усложнить игру. Теперь Тимур и Руслан играют в игру Камень,
# ножницы, бумага, ящерица, Спок. Помогите ребятам вновь бросить честный
# жребий и определить, кто будет делать следующий модуль в новом курсе.
#
# Формат входных данных
# На вход программе подаются две строки текста, содержащие по одному
# слову из перечня "камень", "ножницы", "бумага", "ящерица" или "Спок".
# На первой строке записан выбор Тимура, на второй – выбор Руслана.
#
# Формат выходных данных
# Программа должна вывести результат жеребьёвки: кто победил - Тимур или Руслан,
# или они сыграли вничью.
#
# Примечание. Правила игры стандартные: ножницы режут бумагу.
# Бумага заворачивает камень. Камень давит ящерицу, а ящерица травит Спока,
# в то время как Спок ломает ножницы, которые, в свою очередь,
# отрезают голову ящерице, которая ест бумагу, на которой улики против Спока.
# Спок испаряет камень, а камень, разумеется, затупляет ножницы.

players = [input(), input()]
if players[0] == players[1]:
    print("ничья")
elif ((players[0] == "камень" and players[1] == "ящерица") or
      (players[0] == "камень" and players[1] == "ножницы") or
      (players[0] == "ящерица" and players[1] == "Спок") or
      (players[0] == "ящерица" and players[1] == "бумага") or
      (players[0] == "Спок" and players[1] == "камень") or
      (players[0] == "Спок" and players[1] == "ножницы") or
      (players[0] == "ножницы" and players[1] == "бумага") or
      (players[0] == "ножницы" and players[1] == "ящерица") or
      (players[0] == "бумага" and players[1] == "камень") or
      (players[0] == "бумага" and players[1] == "Спок")):
    print("Тимур")
else:
    print("Руслан")

# камень
# ножницы
# бумага
# ящерица
# Спок

# Руслан
# Тимур
# ничья
